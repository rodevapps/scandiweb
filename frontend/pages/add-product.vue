  ...
  <div style="padding: 10px;">
    <form id="product_form">
      ...
      <div class="form-field">
        ...
        <input id="name" type="text" v-model="form.name" :class="{ error: msg.name !== '' }" required />
        <span v-if="msg.name" class="text-error">{{msg.name}}</span>
      </div>
      <div class="form-field">
        <label for="price">Price ($) </label>
        <input id="price" type="number" min="0.00" step="0.01" v-model="form.price" :class="{ error: msg.price !== '' }" required />
        <span v-if="msg.price" class="text-error">{{msg.price}}</span>
      </div>
      <div class="form-field">
        <label for="type">Type </label>
        <select name="type" id="productType" v-model="form.type" :class="{ error: msg.type !== '' }" required>
      ...
      <div class="form-field" id="dvd" v-if="form.type == 'dvd'">
        ...
        <input id="size" type="number" min="0.00" max="1000000.00" step="0.01" v-model="form.size" :class="{ error: msg.size !== '' }" required />
      ...
      <div class="form-field" id="furniture" v-if="form.type == 'furniture'">
        <div>
          ...
          <input id="height" type="number" min="0.00" max="1000000.00" step="0.01" v-model="form.height" :class="{ error: msg.number !== '' }" required />
          <span v-if="msg.height" class="text-error">{{msg.height}}</span>
        </div>
        <div class="form-field">
          <label for="width">Width (CM) </label>
          <input id="width" type="number" min="0.00" max="1000000.00" step="0.01" v-model="form.width" :class="{ error: msg.width !== '' }" required />
          <span v-if="msg.width" class="text-error">{{msg.width}}</span>
        </div>
        <div class="form-field">
          <label for="length">Length (CM) </label>
          <input id="length" type="number" min="0.00" max="1000000.00" step="0.01" v-model="form.length" :class="{ error: msg.length !== '' }" required />
      ...
      <div class="form-field" id="book" v-if="form.type == 'book'">
        ...
        <input id="weight" type="number" min="0.00" max="1000000.00" step="0.01" v-model="form.weight" :class="{ error: msg.number !== '' }" required />
...
const validate = (element?: string): number => {
    ...
    for (const property in form.value) {
        if (typeof element === "undefined" || property === element) {
            ...
            if ((property === 'price' || property === 'size' || property === 'height' || property === 'width' || property === 'lenght' || property === 'weight') && msg.value[property] === '' && !msg.value[property].match(/^[0-9]+\.[0-9]{2}$/)) {
